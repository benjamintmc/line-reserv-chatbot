...
> ## ⚠️ 歷史文件，內容已不代表現況
>
> 本文件是 2026-07 專案啟動時的**原始需求規格，逐字保留、不再更新**——刻意不改寫，
> 以保存決策起點供追溯。現況與此處出入的部分至少包括：
>
> - **球種**：專案已中性化（D-005 §3），**不預設也不限定球種**；本文件的「高爾夫」字樣屬歷史用語。
> - **授權**：已無「主辦人白名單」；開團全開，關閉／取消限建立者 ∪ 管理員（D-006）。
> - **範圍**：候補（waitlist）與代報名（`+1 名字`）**已納入 MVP** 並上線。
> - **資料庫**：已由 SQLite 移植為 PostgreSQL（D-007／T-012）。
>
> **權威來源**：需求看 `docs/00-project-brief.md`，架構看 `docs/01-architecture.md`，
> 指令與文案看 `docs/02-api-contract.md`，進度看 `docs/task-board.md`。
