...
4. **固定檢查項**（LESSONS 回寫 2026-08-22，皆為已重複 2 次以上的問題）：
   - **改既有 user-facing 文案時**：grep 既有 formatter 中「同一狀態」的措辭，確認未出現第二種
     說法（實例：「已關閉報名」vs「報名已截止」曾並存於相鄰流程）。
   - **設計文件標「釘死」的字串**：與實作**逐字**比對，不做語意比對。不一致時「改實作」或
     「補 errata」二選一必須發生，不得放行——放著會讓下一位讀設計的 agent 把它「修」回去。
