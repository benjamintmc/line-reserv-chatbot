...
| 2026-07-31 | architect-reviewer(D-005) | **MVP 範圍文件（D-001/D-002/D-004）持續被後續 design 增量擴充 + 事後 errata**（D-004 兩次、D-005 對四份文件）。治理成本累積。 | **3** | 觀察中→**達 3 次，提案回寫** | 建議：確立「輕量 errata 協定」或在 schema/parser 設計階段預留演進點；下次階段回報向使用者提案。 |
