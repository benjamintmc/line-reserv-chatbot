...
function openSplit(evt: EventService, venueFee: number): number {
  evt.handleOneline({
    groupId: G,
...
describe('EventService 計費補強（D-005 結算凍結 / OP-1 主辦自移）', () => {
  ...
  it('[D-005 AC-7] split 關閉結算以「凍結後實際正取數」為分母：+6 後 -2 → 正取 5 → ceil(3000/5)=600', () => {
    const { evt, reg } = makeServices(t);
    const eventId = openSplit(evt, 3000);
    // 主辦(1) + 成員 +6 = 7。
    reg.signup({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 6 });
    expect(t.registrations.countConfirmed(eventId)).toBe(7);
    // 成員 -2 → 正取 5（soft-delete，實體列仍在但被過濾）。
    reg.cancel({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 2 });
    expect(t.registrations.countConfirmed(eventId)).toBe(5);
    ...
    expect(t.events.getById(eventId)?.settled_per_person).toBe(600);
  ...
  it('[D-005 AC-15] OP-1 主辦 `-1` 移除自己 → 真實 confirmedCount=0：名單顯示 venue_fee、不 NaN/不 throw', () => {
    const { evt, reg } = makeServices(t);
    const eventId = openSplit(evt, 3000);
    expect(t.registrations.countConfirmed(eventId)).toBe(1);
    // 主辦以 -1 取消自己的第 1 正取（seq=1、owner=host）。
    reg.cancel({ groupId: G, executorLineUserId: HOST, executorDisplayName: '主辦人', messageId: nextMid(), count: 1 });
    expect(t.registrations.countConfirmed(eventId)).toBe(0);
    // 名單費用列以 max(,1) 保底 → 平均每人約 venue_fee 元，且仍標暫估；不得出現 NaN/Infinity。
    const text = listText(reg);
    expect(text).toContain('平均每人約 3000 元（暫估，關閉報名後結算）');
    expect(text).not.toContain('NaN');
    expect(text).not.toContain('Infinity');
  });

  it('[D-005 AC-15] 主辦自移後關閉（confirmedCount=0）：結算 ceil(3000/max(0,1))=3000、不 throw', () => {
    const { evt, reg } = makeServices(t);
    const eventId = openSplit(evt, 3000);
    reg.cancel({ groupId: G, executorLineUserId: HOST, executorDisplayName: '主辦人', messageId: nextMid(), count: 1 });
    expect(t.registrations.countConfirmed(eventId)).toBe(0);
    ...
    expect(t.events.getById(eventId)?.settled_per_person).toBe(3000);
  });
});
