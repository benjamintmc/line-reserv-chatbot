...
import { EventService, type ContinueFlowResult } from './event-service';
...
function step(svc: EventService, text: string): Promise<ContinueFlowResult> {
...
async function walkToFee(svc: EventService): Promise<void> {
  await svc.startCreation({ groupId: G, executorLineUserId: HOST, messageId: next() });
  await step(svc, '2026/08/15');
  await step(svc, '07:30');
  await step(svc, '東方球場');
  const r = await step(svc, '16');
...
describe('create-flow 單題計費（D-005 §6.2 修訂）', () => {
  ...
  it('[D-005 AC-10] 單題「場地費3000」→ split/venueFee=3000/price=0 → awaiting_confirm → 建立', async () => {
    const svc = makeSvc(t);
    await walkToFee(svc);
  ...
  it('[D-005 AC-10] 單題含空白「場地費 3000」（帶空白）→ split/venueFee=3000', async () => {
    const svc = makeSvc(t);
    await walkToFee(svc);
  ...
  it('[D-005 AC-10] 單題「2200」（裸金額）→ per_person/price=2200', async () => {
    const svc = makeSvc(t);
    await walkToFee(svc);
    const fee = await step(svc, '2200');
    expect(fee.kind).toBe('awaiting_confirm');
    const done = await step(svc, '確認');
    expect(done.kind).toBe('created');
    if (done.kind !== 'created') return;
  ...
  it('[D-005 AC-10] 單題「每人2200」與含空白「每人 2200」→ per_person/price=2200', async () => {
    const svc = makeSvc(t);
    await walkToFee(svc);
  ...
  it('[D-005 AC-17] awaiting_fee 無效答案（abc/-1/空）→ field_error 停留、不前進；隨後合法前進', async () => {
    const svc = makeSvc(t);
    await walkToFee(svc);
