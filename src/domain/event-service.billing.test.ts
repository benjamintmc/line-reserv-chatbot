...
function openEvent(
  ...
  const r = evt.confirm({ groupId: G, executorLineUserId: HOST, messageId: nextMid(), hostDisplayName: '主辦人' });
  if (r.kind !== 'created') throw new Error('confirm 未建立');
  return r.event.id;
}

function listText(reg: RegistrationService): string {
  const r = reg.getListView({ groupId: G, messageId: nextMid() }) as ListResult;
...
describe('EventService 計費（D-005 §3–§4）', () => {
  ...
  it('[D-005 AC-3] 主辦自動登記為第 1 正取（seq=1/self/confirmed/owner=host.id/快照名/未取消）', () => {
    const { evt } = makeServices(t);
    const eventId = openEvent(evt, { priceMode: 'per_person', price: 2200 });
    const rows = t.db
      .prepare('SELECT * FROM registrations WHERE event_id = ? ORDER BY seq')
      .all(eventId) as Array<{
      seq: number; kind: string; status: string; owner_user_id: number; display_name: string; cancelled_at: string | null;
    }>;
    expect(rows).toHaveLength(1);
    const host = t.users.getByLineUserId(HOST)!;
    ...
    // 名單第 1 位為主辦。
    expect(t.registrations.listConfirmed(eventId)[0]!.owner_user_id).toBe(host.id);
    expect(t.registrations.countConfirmed(eventId)).toBe(1);
  });

  it('[D-005 AC-4] split +N 後均攤重算：主辦(1) +3 → 正取4 → 每人約 750', () => {
    const { evt, reg } = makeServices(t);
    openEvent(evt, { priceMode: 'split_venue', venueFee: 3000 });
    // 主辦後正取=1（估 3000）。
    expect(listText(reg)).toContain('平均每人約 3000 元（暫估，關閉報名後結算）');
    // 成員 +3 → 正取=4 → ceil(3000/4)=750。
    reg.signup({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 3 });
    expect(listText(reg)).toContain('平均每人約 750 元（暫估，關閉報名後結算）');
  });

  it('[D-005 AC-5] split -N 後均攤重算：正取4(每人750) -1 → 正取3 → 每人約 1000', () => {
    const { evt, reg } = makeServices(t);
    openEvent(evt, { priceMode: 'split_venue', venueFee: 3000 });
    reg.signup({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 3 });
    expect(listText(reg)).toContain('平均每人約 750 元');
    // 成員 -1 → 正取=3 → ceil(3000/3)=1000。
    reg.cancel({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 1 });
    expect(listText(reg)).toContain('平均每人約 1000 元（暫估，關閉報名後結算）');
  });

  it('[D-005 AC-7] split 關閉報名 → 持久化 settled_per_person=429 + 公告最終攤額', () => {
    const { evt, reg } = makeServices(t);
    const eventId = openEvent(evt, { priceMode: 'split_venue', venueFee: 3000 });
    // 主辦(1) + 成員 +6 = 正取7 → ceil(3000/7)=429。
    reg.signup({ groupId: G, executorLineUserId: 'U-m', executorDisplayName: '成員', messageId: nextMid(), count: 6 });
    const close = evt.closeEvent({ groupId: G, executorLineUserId: HOST, messageId: nextMid() });
    expect(close.kind).toBe('ok');
    if (close.kind !== 'ok') return;
    expect(close.confirmedCount).toBe(7);
    expect(close.settledPerPerson).toBe(429);
    // 交易內持久化。
    expect(t.events.getById(eventId)?.settled_per_person).toBe(429);
  ...
  it('[D-005 AC-8] per_person 關閉報名 → settled_per_person 維持 NULL、不附結算列', () => {
    const { evt } = makeServices(t);
    const eventId = openEvent(evt, { priceMode: 'per_person', price: 2200 });
    const close = evt.closeEvent({ groupId: G, executorLineUserId: HOST, messageId: nextMid() });
    expect(close.kind).toBe('ok');
    ...
    expect(t.events.getById(eventId)?.settled_per_person).toBeNull();
    const msg = formatClosed(close.event, close.settledPerPerson, close.confirmedCount).text;
