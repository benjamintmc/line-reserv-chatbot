...
- **MVP 必要功能（對應 requirements §3 功能需求）**：
  ...
  - **FR-4 主辦人授權（2026-07-31 更新，決策 #7）**：雙軌——env `ADMIN_USER_IDS` 為 super-admin（跨群恆管理人）；各群以 `管理人設定`（開啟認領/顯示現況）+ `我是管理人`（認領，先搶先贏、每群上限 3）自助產生在地管理人；生命週期指令（開團/關閉/取消活動）授權 = super-admin ∪ 該群管理人。提供 `我的ID` 讓用戶查自己的 userId。設計 D-006。
  - **FR-5 訊息規範**：只回應可識別指令，其餘忽略；全繁體中文。
- **候補與代報名納入 MVP**（定案 #2、#4，見上 FR-1）。
- **明確不做（Non-goals）**：同群組同時多場活動（MVP 限一場，定案 #3）；球組編排（每組 4 人）與收款統計（v2，定案 #5）；Admin 網頁後台／執行期增刪主辦人指令（**2026-07-31 決策 #7 部分更新**：改為 env super-admin + 群內 `管理人設定`/`我是管理人` 自助認領最多 3 位；仍不做網頁後台）。
...
- **M4** 主辦人授權（群內自助認領）：`管理人設定`（開啟認領/顯示 0/3）、`我是管理人`（認領一席，每群上限 3）、env super-admin 併存、生命週期授權改雙軌；`我的ID` 輔助。設計 D-006（決策 #7）。
...
7. **（2026-07-31 更新，取代 #6 的「僅 env 白名單」）** 主辦人授權改為**雙軌**：
   - **env `ADMIN_USER_IDS` 保留為 super-admin**（跨群、系統擁有者，恆為管理人，不佔各群 3 席）。
   - **群內自助認領**：任一群成員輸入 `管理人設定` 開啟該群管理人認領（顯示現況 0/3 + 說明），成員各自輸入 `我是管理人` 認領一席，**先搶先贏、每群上限 3 位**。這 3 位（＋env super-admin）＝可 `開團`/`關閉報名`/`取消活動` 者。
   - 仍**不做**網頁後台／執行期 Admin 增刪指令（僅「認領」此一自助途徑）。設計見 D-006、里程碑 M4。
