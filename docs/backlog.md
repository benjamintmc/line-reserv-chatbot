...
- **（T-032，2026-08-31 登記）D-019 `編輯 費用` 支援切換計費模式——待使用者核可 APPROVED**：設計文件 `design/D-019-edit-billing-mode.md`（85 行，R2）已完成，**architect-reviewer 與 design-reviewer 雙審皆 PASS、零 blocker**，雙審提出的 4 條 nit（`feeLabel` per_person 標籤統一為「每人費用」、`formatEditBadFee` 措辭改註記「呼應精神非逐字同款」、補呼叫端同步移除 `priceMode` 引數提醒、新增 AC-7 以 spy+grep 佐證 G2 三欄原子寫入非巧合通過）已全數回填。**為什麼現在不做**：使用者要求先自行通讀設計文件全文再核可，尚未裁定 APPROVED。**動工前必須先解**：使用者讀畢後給出 APPROVED（或提出修改意見回architect），orchestrator 才能把此列移回 task-board 看板派工給 backend-engineer。分支 `feat/t-032-edit-billing-mode` 已建立（僅含 task-board 異動，尚無程式碼變更）。內容摘要：`編輯 費用` 沿用開團同款語法（`編輯 費用 2200` / `編輯 費用 場地費3000`）判斷計費模式，任何時候皆可切換（含已有人報名後）；三欄（`price_mode`/`price_per_person`/`venue_fee`）透過新原語 `updateBilling` 單一 UPDATE 原子寫入；mention 行為維持現狀不開例外；反轉 D-015 決議⑥，需回填 D-015 errata（決議⑥、G2、G6、AC-6、`formatEditBadFee` 文案）與 `docs/02-api-contract.md`（見 D-019 §四）。
