...
| D-008 | 單場名額自動釋放（決策 #8）：合併 event_datetime、closed/過期自動釋放、ux_events_active_group active 集合移除 closed、惰性 on-read 過期判定、過期顯示 done | – | R2 | architect | **雙審：architect PASS、design 複審中**（2026-08-02）。**architect-reviewer 零 blocker**（單一 active/flip 原子/三讀取點/0003 等義/errata 全 PASS，6 nit 已採納）。design-reviewer 首審 2 blocker（B1 用詞漂移、B2 名單列組成）**已修訂封閉**（closed 統一「報名已截止」、ended/closed split 去暫估取 perPersonAmount/settled_per_person、移除剩餘名額列），轉 design 複審中。5 Guardrails / 13 AC。**非阻塞微選待問使用者**：ended/closed 剩餘名額列移除(預設) vs 改「報名人數：K」。design 複審過 → 使用者最終 APPROVED → T-014（排 T-012 後） |
