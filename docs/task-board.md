...
## 目前階段：M4 授權簡化 D-006 APPROVED（2026-07-31，R2 雙審通過）— T-011 實作 IN_PROGRESS（含 D-004 授權 errata 回寫）
...
| T-011 | 授權簡化實作（開團移除授權、關閉/取消改認 host_user_id∪super-admin、我的ID 接線、super-admin 空警告）+ D-004 授權 errata | D-006（APPROVED） | R2 | backend-engineer | IN_PROGRESS | src/domain/event-service.ts, src/webhook/handler.ts, src/server.ts, src/index.ts | 2026-07-31 D-006 APPROVED 解阻，已派工。無新 schema/指令 |
