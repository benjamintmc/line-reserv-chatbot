...
## 目前階段：**M5 完成待部署**。T-012（PG 移植）DONE + T-014（單場自動釋放，D-008）DONE，R2 全通過、271 tests 綠、AC 142/142。**下一步＝使用者依 `docs/deployment-runbook.md` 部署**（Neon 建DB → 直連 migrate 三檔含 0003 → build/push image → Cloud Run deploy → LINE webhook → 冒煙測試）。部署需使用者 Neon + Google Cloud 帳號 + gcloud CLI
