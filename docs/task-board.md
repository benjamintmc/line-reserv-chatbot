...
## 目前階段：M4 授權簡化（D-006 改採「開團人擁有 + super-admin 安全網」，作廢管理人認領方案）。brief 決策 #7 定案。R2；D-006 改寫後雙審 → T-011 實作
...
| T-011 | 授權簡化實作（開團移除授權、關閉/取消改認 host_user_id∪super-admin、我的ID 接線）+ D-004 授權 errata | D-006（改寫中） | R2 | backend-engineer | BLOCKED | src/domain/event-service.ts, src/webhook/handler.ts, src/server.ts | 等 D-006 APPROVED。無新 schema/指令；比原 D-006 小很多 |
