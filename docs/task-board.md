...
| T-014 | 單場自動釋放實作（migration 0003 合併 event_datetime + 改 ux active 集合、event-service 過期判定/開團 flip、findOpenEventForSignup/findEventForDisplay 三讀取點、phase 名單 formatter、create-flow 存 UTC datetime、鎖內 getById 重讀）＋套用預列 errata（D-001/D-003/D-004/D-005/D-007） | D-008（APPROVED） | R2 | backend-engineer | **實作完成+自驗綠，R2 三關審查中**（2026-08-02）。6 commit（0517148~ed53ec0）：0003 migration、event-status.ts、三讀取點+flip、formatter phase、測試遷移、errata。自驗 **37 檔 268 tests 綠、build/lint 0、AC 142/142**（真 PG）。errata 套 D-003/D-004/D-005，D-001/D-007 加註待 architect 追認。**R2 三關並行**：architect-reviewer（實作 vs D-008 + 追認 D-001/D-007 errata）+ unit-tester（AC-2/6/9 真並行/邊界）+ design-reviewer（phase 名單文案）。⚠️行為變更（依設計）：closed 不再能取消活動→no_active。三關過 → 部署 |
