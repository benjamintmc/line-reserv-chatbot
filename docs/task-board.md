...
| T-012 | PG 移植實作（driver/repositories/migrations/serverless/Dockerfile/config） | D-007（APPROVED） | R2 | backend-engineer | **修 B1 blocker 中**（2026-08-01）。**unit-tester PASS**（AC-2/3/12 真驗+反例、零產品碼 bug、補連線洩漏/多事件測試，255 綠、`ff9cbaf`）。**architect-reviewer：G1~G7 全 PASS、AC 實質驗證、N-new-1/2 落地，惟 1 blocker B1**——cancel 的 `freedConfirmed` 交易外快照當決策輸入，sync→async 激活超賣窗（並發雙 cancel 同列→多遞補→正取>capacity）；根在 D-003/D-007 §3、T-006 nit-2 預言成真。修法：交易內由實際取消的 confirmed 列數推導。**派 backend 修 B1+回歸測試 → architect-reviewer 複審 B1 → 部署 runbook → DONE** |
| D-008 | 單場名額自動釋放（決策 #8）：合併 event_datetime、closed/過期自動釋放、ux_events_active_group active 集合移除 closed、惰性 on-read 過期判定、過期顯示 done | – | R2 | architect | **OP 全定案，architect 更新設計正文中→待雙審**（2026-08-01）。7 OP 定案：**OP-1 独立 0003（post-T-012）**、**OP-4 closed 顯示最終名單標「已截止」**（findEventForDisplay 納入 closed；done→「活動已結束」/closed→「報名已截止」兩顯示狀態）、OP-2/3 拒絕文案+已結束措辭採預設、OP-5 draft 無需特別處理、OP-6 固定 UTC+8、OP-7 close/cancel 遇過期 open 回 no_active 不 flip。判定式 `NOW()>event_datetime OR status IN(closed,cancelled,done)`；索引拆兩半（0003 移除 closed + 開團內過期 open flip done）。errata：D-007「無 0003」→新增 0003；牽動 D-001/D-003/D-004/D-005 |
