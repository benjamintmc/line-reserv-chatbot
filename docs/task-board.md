...
| T-014 | 單場自動釋放實作（migration 0003 合併 event_datetime + 改 ux active 集合、event-service 過期判定/開團 flip、findOpenEventForSignup/findEventForDisplay 三讀取點、phase 名單 formatter、create-flow 存 UTC datetime、鎖內 getById 重讀）＋套用預列 errata（D-001/D-003/D-004/D-005/D-007） | D-008（APPROVED） | R2 | backend-engineer | **實作中途，session 上限中斷（未提交、未驗證）**（2026-08-02，重置 08:10）。**工作目錄 WIP**：新增 `0003_merge_event_datetime.sql`、`src/domain/event-status.ts`；改 schema/time/event-repository/event-service/registration-service/event-formatter/list-formatter/handler + 多測試檔。中斷於改測試 fixture（migrate.test count=3、closed 釋放反轉、event_datetime 建構）。**未跑測試驗證**。**下一步：額度恢復後恢復 backend agent 收尾測試+驗證 → R2 三關**。⚠️ WIP 未提交、勿 git add -A |
