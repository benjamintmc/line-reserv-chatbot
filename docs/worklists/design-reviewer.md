# Worklist — design-reviewer

> 擁有者：design-reviewer（唯一可寫者）。Orchestrator 可讀不可寫，透過派工單指派任務。
> 這裡是你的工作區：佇列、筆記、疑問、提議。**狀態真相在 `docs/task-board.md`，以那裡為準。**
>
> **本專案的職能範圍**：無前端 UI，你審的是 **LINE 對話文案／UX**——訊息措辭一致性、
> 使用者可見狀態詞彙、無效輸入的重問範本、拒絕回覆是否有出口。D-004~D-008 皆循此模式。
