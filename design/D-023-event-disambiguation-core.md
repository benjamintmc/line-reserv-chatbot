...
## errata（2026-09-02，來源 T-033a 實作揭露的內部歧義；**使用者已裁決，本節生效**）

> **§4.3 `matchSelector` 比對規則步驟 2 與步驟 4 對「月日 token」的年份處理不一致**——步驟 2 寫
> 「無年，**用今年份**比對台灣本地日期的 `MM-DD`」，步驟 4 寫「月日 token 只比對 `MM-DD` 後兩段，
> **忽略年份**」。單一年度內兩者等價，故切檔前歷次審查未察；**跨年時結果不同**：若今日為 2026-12-30、
> 群組有一場 2027-01-05 的活動，依步驟 4 打 `@旭陽 1/5` 命中，依步驟 2 則 `1/5` 解析為 2026-01-05
> 而命中不到。由 backend-engineer 於 T-033a 實作時發現並上報（未自行改設計）。
>
> **使用者裁決（2026-09-02）：採步驟 4「忽略年份」**，理由是跨年開團在實務上常見，要求使用者為此
> 改打完整日期是不必要的負擔。逐點更正：
>
> 1. **步驟 2 中「用今年份比對台灣本地日期的 `MM-DD`」廢止**，更正為「無年；僅取 `MM-DD` 兩段，
>    與候選活動的台灣本地日期 `MM-DD` 比對，**不解析為任何特定年份**」。步驟 4 文字不變、為準。
> 2. **`matchSelector` 的 `now` 參數因此不參與任何判斷**。簽名屬 §4.3 釘死值，**維持不變**
>    （`resolveTargetEvent` 仍逐字依原簽名轉呼叫），實作以 `void now;` 明示；**不得**為此刪改簽名。
> 3. 已知代價（**接受，非缺陷**）：若同群同時存在 2026-01-05 與 2027-01-05 兩場活動，`@1/5` 會命中
>    兩場而回 `too_many`，使用者需改打完整日期 `@2027/1/5` 窄化——此為 `too_many` 的既定用途，
>    非新缺陷。
>
> **本節由 orchestrator 落筆記錄使用者裁決，尚未經 architect 確認**；已列入 T-033a 的 R2 雙審待確認項。
