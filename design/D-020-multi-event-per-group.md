...
| 日期 | 議題 | 狀態 |
|---|---|---|
| 2026-09-02 | D-021 過渡條文的單獨複驗（交接快照 §3.1 待辦，標的：D-021 L94-105 開團側過渡條文 + L124-125 AC-5 改隸） | **1 blocker + 2 concern → 已全數落檔封閉**。
  緣由：前一輪 R2 雙審的 5 個 blocker 由 orchestrator 逐條修訂，但**修訂後的新條文無 reviewer 看過**；
  依交接快照 §3.1 建議請 architect-reviewer 單獨複驗。**B1（成立，已修）**：`actives.at(-1)` 的「id 升冪」
  保證原本只寫在 §2 的 JSDoc 註解裡，§2 未給 SQL、AC-2 亦不驗順序，而現況 `findActiveByGroup` 是
  `ORDER BY id DESC LIMIT 1`（`event-repository.ts:106-114`）——實作者刪 `LIMIT 1` 而留 `DESC` 即取到最舊，
  正是 clause 2 明文要防的靜默行為變更，卻無機制可擋 ⇒ §2 釘死 `ORDER BY id ASC`，AC-2 補升冪斷言並標明
  **驗證層級為 repository 層**（走 `開團` 流程構造第二場依不變式 #1 必被應用層擋下 ⇒ 假紅）。
  **C-B（成立，已修）**：G1 逐字禁止 `listActiveByGroup(groupId)[0]`，與過渡條文的 `.at(-1)` 文面打架 ⇒
  於 **G1 本體**加 carve-out（限三函式、限 T-033a~b、不得抽共用函式），而非在 §1 另立例外——後者會被讀成
  guardrail 遭架空。**C-C（成立，已修）**：移除條件原寫「`.at(-1)` **這類**取用」不可機械化且可被
  `actives[actives.length-1]` 繞過 ⇒ 點名三個函式並附可執行 grep。
  **⚠ B2「clause 4 與不變式 #2 直接矛盾」經往返後由 reviewer 自行撤回，降級為文字歧義（CONCERN）**——
  **不變式 #2 未被推翻，後續 agent 不得據 RP 中的 BLOCK 字樣重開此議**。撤回理由：clause 4 開頭
  「**若**同時存在 2 場過期 active」已為該情形指定 flip 行為，宣稱不可達的條文不會先為它定義行為；且
  **同段 L97 已用過一次「現況」**指 0006 前的既有程式行為，同段兩次「現況」應同指涉，故該括號指 0006 前
  基線、事實正確。reviewer 並據此把 §4「新暴露路徑」固定檢查項的結論改判為 **PASS**。
  惟 clause 4 確有實質缺漏（稱殘留場「可由 `關閉報名` 清除」，漏了不變式 #2(a) 的「多場時需以 `@selector`
  指定」前提）⇒ 採 reviewer 自行改議的**輕改法**（撤回其原提的整段替換，因整段替換會洗掉「與現況邏輯
  一致」這個正確的等價性論證並引入跨檔引用），恢復手段**就地內聯**、不引用 umbrella。
  **落檔**：由 **architect** 執行（`OWNERSHIP.md:28`，§一/§二/§三 歸撰寫該設計的 agent），共 7 條
  （上述 5 條 + G1 carve-out + AC checkbox 標籤 `[D-020 AC-n]`→`[D-021 AC-n]` 防呆 nit；後者不影響
  `check_ac_coverage` 判定〔文件編號取自檔名〕，純為防止實作者照抄標籤寫錯測試標記）。
  D-021 由 134→145 行。**orchestrator 親自核驗 diff 逐條相符並實跑關卡**：AC 覆蓋 **249/249**、
  board_sync ✓、doc_budget ✓（D-020 146 行／D-021 144 行印 ℹ，屬規則四預期內，**不得據以再拆**）。 |
