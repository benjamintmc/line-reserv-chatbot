 ...
 *
 * M2 跨試輔助：手動建立一場 status='open' 的活動，讓 M2 報名核心（+N/-N/名單）
 * 能在真實 LINE 群組上跨試（開團流程屬 M3/D-004，尚未實作）。
 *
 * 用法（PowerShell）：
 *   $env:GROUP_ID='Cxxxxxxxx...'; $env:HOST_LINE_USER_ID='Uxxxxxxxx...'; npm run db:seed
 *
 * 環境變數：
 ...
 *   DATABASE_PATH      （選填，預設 ./data/golf.db；與 server 同一檔才有效）
 *
 * 安全：若該群組已有 active 活動（draft/open/closed），本腳本不重複建立，直接印出既有活動並結束
 * （避免觸發 ux_events_active_group 唯一約束）。要重新來過請先刪除 DATABASE_PATH 檔或改該活動狀態。
 */
...
function main(): void {
  ...
  const existing = events.findActiveByGroup(groupId);
  if (existing !== undefined) {
    console.log('⚠️  此群組已有 active 活動，未重複建立。既有活動：');
    console.log(JSON.stringify(existing, null, 2));
    console.log(
      '\n若要重來：刪除 DATABASE_PATH 檔（預設 ./data/golf.db）或將該活動狀態改為 cancelled 後再跑本腳本。',
    );
    return;
  }

  const host = users.upsert(hostLineUserId, hostDisplayName);
